...
class AllSettingsDialog(QtWidgets.QDialog):
    ...
    def setDefaultValues(self):
        """
        Sets values of settings dialog to default values.
        *** NOTE: There is no error checking in this method!
        If default settings are changed, make sure they are correct / valid! ***
        """
        # System Settings:
        if self.defaultSettings['system_settings']['system'] == "Kongsberg":
            self.ui.radioButtonKongsberg.setChecked(True)
        else:
            self.ui.radioButtonOther.setChecked(True)
            self.ui.lineEditOther.setText(self.defaultSettings['system_settings']['system'])

        # IP Settings:
        self.ui.lineEditIP.setText(self.defaultSettings['ip_settings']['ip'])
        self.ui.lineEditPort.setText(str(self.defaultSettings['ip_settings']['port']))

        # Processing Settings:
        self.ui.doubleSpinBoxBinSize.setValue(self.defaultSettings['processing_settings']['binSize_m'])
        self.ui.doubleSpinBoxAcrossTrackAvg.setValue(self.defaultSettings['processing_settings']['acrossTrackAvg_m'])
        self.ui.doubleSpinBoxDepth.setValue(self.defaultSettings['processing_settings']['depth_m'])
        self.ui.doubleSpinBoxDepthAvg.setValue(self.defaultSettings['processing_settings']['depthAvg_m'])
        self.ui.spinBoxAlongTrackAvg.setValue(self.defaultSettings['processing_settings']['alongTrackAvg_ping'])
        if self.defaultSettings['processing_settings']['dualSwathPolicy'] == 0:
            self.ui.radioButtonAllPings.setChecked(True)
        elif self.defaultSettings['processing_settings']['dualSwathPolicy'] == 1:
            self.ui.radioButtonFirstPing.setChecked(True)
        elif self.defaultSettings['processing_settings']['dualSwathPolicy'] == 2:
    ...
    def validateAndloadValues(self, loadedSettings):
        ...
        if loadedSettings['ip_settings']['ip'] != self.settings['ip_settings']['ip']:
            try:
                ...
                ipaddress.ip_address(loadedSettings['ip_settings']['ip'])
        ...
        if loadedSettings['processing_settings']['depth_m'] != self.settings['processing_settings']['depth_m']:
            self.settings['processing_settings']['depth_m'] = loadedSettings['processing_settings']['depth_m']
    ...
    def validateAndSetValues(self):
        ...
        if self.settings['processing_settings']['binSize_m'] != self.ui.doubleSpinBoxBinSize.value():
            print("settings not equal", self.settings['processing_settings']['binSize_m'])
        ...
        if self.settings['processing_settings']['acrossTrackAvg_m'] != self.ui.doubleSpinBoxAcrossTrackAvg.value():
            self.settings['processing_settings']['acrossTrackAvg_m'] = round(self.ui.doubleSpinBoxAcrossTrackAvg.value(), 2)
            # self.acrossTrackAvgEdited.emit()
            acrossTrackAvgEdited = True
        # depth_m:
        if self.settings['processing_settings']['depth_m'] != self.ui.doubleSpinBoxDepth.value():
            self.settings['processing_settings']['depth_m'] = round(self.ui.doubleSpinBoxDepth.value(), 2)
            depthEdited = True
        # depthAvg_m:
        if self.settings['processing_settings']['depthAvg_m'] != self.ui.doubleSpinBoxDepthAvg.value():
            self.settings['processing_settings']['depthAvg_m'] = round(self.ui.doubleSpinBoxDepthAvg.value(), 2)
            # self.depthAvgEdited.emit()
            depthAvgEdited = True
        # alongTrackAvg_ping:
        if self.settings['processing_settings']['alongTrackAvg_ping'] != self.ui.spinBoxAlongTrackAvg.value():
            self.settings['processing_settings']['alongTrackAvg_ping'] = int(self.ui.spinBoxAlongTrackAvg.value())
            # self.alongTrackAvgEdited.emit()
            alongTrackAvgEdited = True
        # dualSwathPolicy (0 to keep all pings; 1 to keep first ping only; 2 to keep second ping only):
